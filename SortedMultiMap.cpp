...
void SortedMultiMap::add(TKey c, TValue v) {
    int where = this->hash_function_1(c);
    ...
    if (now == nullptr){
            this->resize();
            where = this->hash_function_1(c); //recalculates the hash for key because capacity was changed
            now = this->dict[where];
            prev = nullptr;
            while (now != nullptr) {
                if (comp(now->key, c) && now->key != c) {
...
void SortedMultiMap::resize() {
    ...
    for (int i = 0; i < this->capacity/2; ++i){
        Node* now = this->dict[i];
        while (now != nullptr){
            int where = this->hash_function_1(now->key);
            Node* next = now->next;
            now->next = aux[where];
            aux[where] = now;
            now = next;
